"""
LLM integration for RAG Phase 2.
Uses OpenRouter API and falls back to deterministic context summaries when unavailable.
"""
...
class LocalLLMEngine:
    """LLM helper that prefers OpenRouter and falls back gracefully."""

    def __init__(self) -> None:
        self.enabled = ENABLE_RAG_LLM_SUMMARY
        self.provider = LLM_PROVIDER
        self.model = OPENROUTER_MODEL
        # OpenRouter settings are read from config or environment
        self.openrouter_key = os.environ.get("OPENROUTER_API_KEY")
        self.openrouter_base = OPENROUTER_BASE_URL.rstrip("/")
    ...
    def _build_prompt(self, batch_id: str, eval_report: Dict, rag_context: Dict) -> str:
        ...
        prompt = (
            ...
            "Write for a QA operator who needs a fast decision aid. "
            "Be concise, factual, and do not invent unsupported causes. "
            "Prefer concrete process checks over abstract explanations.\n\n"
            f"Batch ID: {batch_id}\n"
            ...
            "SUMMARY: <2-3 sentences; state what the batch decision means, whether this looks like a recurring pattern or an isolated case, and what the operator should know right now>\n"
            "LIKELY_CAUSES: <ranked causes separated by semicolons; each item should mention evidence from rules, similarity, shift, or feedback. Use 'Unknown' only if there is no evidence>\n"
            "RECOMMENDED_ACTIONS: <ordered actions separated by semicolons; each action must be specific, measurable, and actionable on the factory floor>\n"
            "CONFIDENCE: <Low|Medium|High; choose Low if the evidence is weak or mixed, Medium if the pattern is suggestive, High only if history and rules strongly agree>"
        )
        return prompt
    ...
    def _fallback_insight(self, batch_id: str, eval_report: Dict, rag_context: Dict) -> InsightResult:
        ...
        if similar_batches:
            strongest = max(similar_batches, key=lambda item: item[1])
            strongest_decision = strongest[2].get("decision", "UNKNOWN")
            summary_text = (
                f"{summary_text} The closest match is a {strongest_decision} case, so the current decision should be read as part of a repeating pattern rather than a one-off event."
            )
    ...
    def _parse_llm_text(self, response_text: str) -> InsightResult:
        summary = ""
        likely_causes = ""
        recommended_actions = ""
        confidence = "Medium"
        ...
        if isinstance(data, dict):
            ...
            if choices and isinstance(choices, list):
                ...
                if isinstance(msg, dict):
                    return msg.get("content") or msg.get("text") or json.dumps(data)
        ...
        if provider == "openrouter":
            try:
                ...
                if not response_text:
                    ...
                    return self._fallback_insight(batch_id, eval_report, rag_context).to_dict()
                return self._parse_llm_text(response_text).to_dict()
            except Exception as exc:
                logger.warning(f"OpenRouter call failed ({exc}); using fallback insight.")
                return self._fallback_insight(batch_id, eval_report, rag_context).to_dict()

        logger.info("RAG LLM provider not recognized; using fallback insight generation.")
        return self._fallback_insight(batch_id, eval_report, rag_context).to_dict()